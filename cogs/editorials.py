...
class Editorials(commands.Cog):
    ...
    async def editorials(self, ctx):
        ...
        for i in range(len(editorial_list) // 5 + 1):
            embed = discord.Embed(
                title='__**Editorials**__',
                description='',
                colour=discord.Colour.blurple(),
            )
            embed.set_author(name='Codechef',
                             icon_url='https://static.dribbble.com/users/70628/screenshots/1743345/codechef.png')
        ...
        for i in range(len(editorial_list)):
            if (i % 5 == 0 and i != 0):
                current_embed_num += 1
                current_embed = embed_list[current_embed_num]
            current_embed.add_field(name=str(i+1)+'. '+''+''+editorial_list[i][0]+''+'',
                                    value='[`Find the editorials here`]({})'.format(editorial_list[i][1]),
                                    inline=False)
